...
import utils.__hw_control as __hw_control
...
def draw_detections(frame, detections, palette, labels, output_transform):
    ...
    palette_2 = [(176, 7, 227), (66, 247, 30), (133,199,234)]

    for detection in detections:
        class_id = int(detection.id)
        #color = palette[class_id]
        color = palette_2[class_id]
        det_label = labels[class_id] if labels and len(labels) >= class_id else '#{}'.format(class_id)
        xmin, ymin, xmax, ymax = detection.get_coords()
        # ROI Setting
        ROI = [190, 120, 350, 370]
        if ROI[0]<= xmin and xmax<= ROI[2]:
            frame_cpy = frame.copy()
            rect = frame_cpy[ymin:ymax, xmin:xmax]
            cv2.rectangle(frame, (xmin, ymin), (xmax, ymax), color, 6)
            cv2.rectangle(frame, (xmin, ymin), (xmax, ymax), color, -1)
            alpha = 0.3
            frame[ymin:ymax, xmin:xmax] = cv2.addWeighted(frame[ymin:ymax, xmin:xmax], alpha, rect, 1 - alpha, 0)

            cv2.putText(frame, '{} {:.1%}'.format(det_label, detection.score),
                        (xmin, ymin - 10), cv2.FONT_HERSHEY_COMPLEX, 1, color, 2)
...
def print_raw_results(detections, labels, frame_id):
    ...
    for detection in detections:
        ...
        class_id = int(detection.id)
        det_label = labels[class_id] if labels and len(labels) >= class_id else '#{}'.format(class_id)
        log.debug('{:^9} | {:10f} | {:4} | {:4} | {:4} | {:4} '.format(det_label, detection.score, xmin, ymin, xmax, ymax))

def gen_mask(frame, threshold=128):
...
def main():
    ...
    cap = open_images_capture(args.input, args.loop, camera_resolution=(640, 480))
    ...
    # # # # # # # # # # # # # #
    inference_period = 5
    detected_obj = {0:'TRIANGLE',1:'RECTANGLE',2:'CIRCLE'}
    area_cnt =0
    # # # # # # # # # # # # # #

    hw_control = __hw_control.hw()
    hw_control.start()
    while True:
        ...
        if results:
            ...
            class_id, frame = draw_detections(frame, objects, palette, model.labels, output_transform)

            if class_id != None:
                obj_name = detected_obj.get(class_id)
                print('{} is Detected!!'.format(obj_name))
                if obj_name =='TRIANGLE':
                    area_cnt = area_cnt + 1
                    if area_cnt >=3:
                        print('{} is in ROI!'.format(obj_name))
                        hw_control.path_C()
                        area_cnt = 0
                if obj_name == 'RECTANGLE':
                    area_cnt = area_cnt + 1
                    if area_cnt >= 3:
                        print('{} is in ROI!'.format(obj_name))
                        hw_control.path_B()
            ...
            next_frame_id_to_show += inference_period
        ...
        if detector_pipeline.is_ready() and next_frame_id % inference_period == 0:
            # Get new image/frame
